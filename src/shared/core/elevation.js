...
function shadow(z) {
  return [
    `${umbraMap[z]} rgba(${baselineColor} / ${umbraOpacity})`,
    `${penumbraMap[z]} rgba(${baselineColor} / ${penumbraOpacity})`,
    `${ambientMap[z]} rgba(${baselineColor} / ${ambientOpacity})`,
  ].join(', ')
}
...
function overlay(z) {
  ...
  const overlayColor = `rgba(var(--color-on-surface) / ${overlayOpacity})`
 ...
 * @param {number} z
 */
export default function elevation(z) {
  return css`
    z-index: ${z};
